                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead>
                        <tr>
                            ...
                            <th class="px-6 py-3 text-left text-xs font-medium uppercase">Mais informação</th>
                    ...
                    <tbody>
                        @foreach ($records as $punch)
                            <tr>
                                <td class="px-6 py-4 text-sm">{{ \Carbon\Carbon::parse($punch->date)->format('d/m/Y') }}
                                </td>
                                <td class="px-6 py-4 text-sm">
                                    {{ \Carbon\Carbon::parse($punch->entry_time)->format('H:i') }}</td>
                                <td class="px-6 py-4 text-sm">
                                ...
                                <td class="px-6 py-4 text-sm">
                                    <a href="{{ route('record.show', $punch->id) }}"
                                        class="inline-flex items-center px-4 py-2 bg-green-500 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-green-700 active:bg-green-900 focus:outline-none focus:border-green-900 focus:ring ring-gren-300 disabled:opacity-25 transition ease-in-out duration-150">Visualizar</a>
                                </td>
                                <td class="px-6 py-4 text-sm">
